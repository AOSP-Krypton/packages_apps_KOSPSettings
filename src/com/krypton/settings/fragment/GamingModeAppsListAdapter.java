...
public class GamingModeAppsListAdapter extends ListAdapter<AppInfo, AppInfoViewHolder> {
    ...
    public GamingModeAppsListAdapter(Context context) {
        super(mItemCallback);
        mContext = context;
    }
    ...
    public AppInfoViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        return new AppInfoViewHolder(LayoutInflater.from(mContext).inflate(
            R.layout.gamingmode_apps_list_item, viewGroup, false));
    }
    ...
    private void updateAppsListInSettings(String key, boolean checked) {
        String list = Utils.getStringFromSettings(mContext, GAMINGMODE_APPS);
        if (list != null) {
        ...
        Utils.putStringInSettings(mContext, GAMINGMODE_APPS, list);
    }
}
