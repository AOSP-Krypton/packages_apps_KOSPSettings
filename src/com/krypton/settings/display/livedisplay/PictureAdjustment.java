...
package com.krypton.settings.display.livedisplay;
...
import com.krypton.settings.preference.CustomDialogPreference;
...
public class PictureAdjustment extends CustomDialogPreference<AlertDialog> {
    ...
    private class ColorSeekBar implements SeekBar.OnSeekBarChangeListener {
        ...
        public ColorSeekBar(SeekBar seekBar, Range<Float> range, TextView value, int index) {
            mSeekBar = seekBar;
            mValue = value;
            mIndex = index;
            mRange = range;
            mSeekBar.setMin(Math.round(range.getLower() * 100));
            mSeekBar.setMax(Math.round(range.getUpper() * 100));
        ...
        public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
            if (fromUser) {
                mCurrentAdj[mIndex] = mRanges.get(mIndex).clamp(progress / 100f);
                updateAdjustment(mCurrentAdj);
            }
